class FormMailer < ActionMailer::Base
  ...
  def confirm(reply)
    @reply = reply
    email = [reply.user.email].join(';')
    title = reply.form.email_confirm_subject.presence || reply.form.title.presence || reply.form.path

    mail(to: email, subject: [Formgen.subject_prefix.presence, t('form_mailer.notify.new_notification_for', title: title)].join(" ").strip, template_name: 'confirm')
  end
